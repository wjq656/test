  ...
  <body>
     <div id="loginbox">
		<form id="loginform" action="" method="post">
			<div class="normal_text"> <h3><img src="${bathPath}/resources/images/logo.png" alt="Logo">管理后台</h3></div>
			<div class="control-group">
				<div class="controls">
					<div class="main_input_box">
						<span class="add-on bg_lg"><i class="fa fa-user"></i></span><input type="text" id = "email" autofocus="autofocus"  placeholder="请输入账号...">
					</div>
				</div>
			</div>
			<div class="control-group">
				<div class="controls">
					<div class="main_input_box">
						<span class="add-on bg_ly"><i class="fa fa-lock"></i></span><input type="password"  id="password" placeholder="请输入密码...">
					</div>
				</div>
			</div>
			<div class="form-actions">
				<span><a type="submit"  class="btn btn-success" onclick="login()">登陆</a></span>
			</div>
		</form>
	</div>
	...
	<script type="text/javascript">
		...
		$("#email").keydown(function(e){
				wjqStorage.set("email",$(this).val(),1)
			})
						...
						alert("用户名或密码错误");
							...
							alert("请输入账户名密码");
								...
								alert("该账户被禁止登录")
									...
									window.location.href=adminPath+"/index?name=111";
		...
		var wjqStorage = {
			set:function(key,value,mark){
				if(window.localStorage){
					var storage = mark==1?window.sessionStorage:window.localStorage;
					storage.setItem(key,value);
				}else{
					//Cookie.write("menuTitle", arrDisplay);
					//cookie  表示如果用storage不行就用cookie 缓存技术
				}
			},
